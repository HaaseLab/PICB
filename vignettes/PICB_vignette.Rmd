...
In the [Appendix](#appendix), we provide a short bash script to check if your BAM file meets above PICB requirements.
...
For the demo, we are using a subset of our mapped small RNAs from Drosophila ovaries [SRR7346241](https://www.ncbi.nlm.nih.gov/sra/?term=SRR7346241). In short, we trimmed the 3’ adaptor and filtered for reads ≥24 nucleotides. 
...
```{r example_load_genome, results="hide", message=FALSE}
library("BSgenome.Dmelanogaster.UCSC.dm6")
```

Now, let's store the genome into the variable `myGenome`.
```{r example_store_genome}
myGenome <- "BSgenome.Dmelanogaster.UCSC.dm6"
```

Instead you can also use any of the other provided options to load the corresponding reference genome. We demonstrated the other options in the [Reference Genome](#reference-genome) section.
...
As described in the beginning of this [Example Workflow](#example-workflow), we subsetted the dataset to only include reads from chromosome 2L and further downsampled the file to retain 22% of the reads for the sake of this demonstration. 
...
## Unsure how the bam-files were mapped?

You can test if your bam file has the NM and NH tags and is coordinate-sorted using the samtools package [Li et al., 2009](https://doi.org/10.1093/bioinformatics/btp352):
```{bash BAMtest, eval = FALSE}
# in bash terminal
# NH and NM tags present?
